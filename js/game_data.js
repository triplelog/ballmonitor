...
function loadGames() {
	...
	if (borp == 'p'){
		request.open('GET', 'http://localhost:8000/gamesp?formula='+streakid, true);
	}
	else {
		request.open('GET', 'http://localhost:8000/games?formula='+splitstr+streakid, true);
	}
	request.onload = function() {
	  ...
	  if (request.status >= 200 && request.status < 400) {
		...
		myData = [];
		if (data['games'][0][1].split.name != 'None') {
			for (var i=0;i<data['games'].length;i++){
				myData.push([data['games'][i][1].dates, data['games'][i][1].split.value,shiohplayers[data['games'][i][1].stats[0]][0],data['games'][i][0],data['games'][i][1].stats[4]]);
			}
	
			var columns = ['Dates',data['games'][0][1].split.name,'Name',streakid.toUpperCase(),'PA'];
			
		}
		else {
			for (var i=0;i<data['games'].length;i++){
				myData.push([data['games'][i][1].dates, shiohplayers[data['games'][i][1].stats[0]][0],data['games'][i][0],data['games'][i][1].stats[4]]);
			}
...
function createGames(columns) {
	...
	var tbody = table.append('tbody').attr('id','gamesBody');
...
function makeChart(statcol) {
	...
	for (var i=0;i<20;i++) {
		var sparkline1 = new Sparkbar(gamesBody.querySelectorAll('tr')[i].querySelectorAll('td')[statcol]);
		var statVals = [];
		var myData = data['games'][i][1];
